...
void printAp(string file_name, vector<double> vals[]){
  ...
  for (int v = 0; v < 3; ++v)
    for (int i = 0; i < vals[v].size(); i = i + 4)
      sum[v] += vals[v][i];
  printf(",%f,%f,%f", sum[0] / 11 * 100, sum[1] / 11 * 100, sum[2] / 11 * 100);
...
bool eval(string gt_dir, string result_dir, string plot_dir, Mail* mail, string num){
...
//  string plot_dir       = result_dir + "/plot_05_iou";
...
//  system(("mkdir " + plot_dir).c_str());
  ...
  std::vector<int32_t> indices = getEvalIndices(result_dir + "/data_" + num + "/");
  ...
  for (int32_t i=0; i<indices.size(); i++) {
    ...
    vector<tDetection>   det  = loadDetections(result_dir + "/data_" + num + "/" + file_name,
...
//  mail->msg("  done.");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_05_iou_" + num +".txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_05_iou_" + num +".txt").c_str(),"w");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_ground_05_iou_" + num +".txt").c_str(), "w");
      vector<double> precision[3], aos[3], aos_ground[3];
...
int32_t main (int32_t argc,char *argv[]) {
  ...
  string name = argv[3];
  string num = argv[4];
  string plot_dir = argv[5];

  // init notification mail
  Mail *mail;
  mail = new Mail();
  //mail->msg("Thank you for participating in our evaluation!");
  printf("\nexperiment_name,checkpoint,class,detection_AP,detection_moderate,detection_hard,"
           "detection_BEV_AP,detection_BEV_moderate,detection_BEV_hard,"
           "heading_BEV_AP,heading_BEV_moderate,heading_BEV_hard,"
           "detection_3D_AP,detection_3D_moderate,detection_3D_hard,"
           "heading_3D_AP,heading_3D_moderate,heading_3D_hard\n");
  printf("%s,%s", name.c_str(), num.c_str());
  // run evaluation
  if (eval(gt_dir, result_dir, plot_dir, mail, num)) {
