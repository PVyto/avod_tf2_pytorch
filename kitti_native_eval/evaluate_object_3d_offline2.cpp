...
void saveAndPlotPlots(string dir_name,string file_name,string obj_type,vector<double> vals[],bool is_aos) {
    ...
    printf(",%f, %f, %f", sum[0] / 40 * 100, sum[1] /40 * 100, sum[2] / 40 * 100);
...
//  mail->msg("  done.");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    ...
    if (eval_image[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_" + num + ".txt").c_str(), "w");
      if(compute_aos)
        fp_ori = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_orientation_" + num + ".txt").c_str(),"w");
  ...
  for (int c = 0; c < NUM_CLASS; c++) {
    CLASSES cls = (CLASSES)c;
    if (eval_ground[c]) {
      fp_det = fopen((result_dir + "/stats_" + CLASS_NAMES[c] + "_detection_ground_" + num + ".txt").c_str(), "w");
      vector<double> precision[3], aos[3], aos_ground[3];
