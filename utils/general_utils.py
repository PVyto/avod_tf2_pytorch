...
def mean_unorm_image(image):
    from data_utils.transforms import Unormalize
    image = image.squeeze(0).permute(1, 2, 0)
    mean_unorm = Unormalize()
...
def normalize_labels(original_labels, extents):
    ...
    # labels = original_labels.cpu().numpy().copy()
    # else:
...
def unormalize_labels(original_labels, extents):
    # labels = original_labels.numpy().copy()
    labels = original_labels.clone()
    labels[:, [0, 2]] *= extents[1]
    labels[:, [1, 3]] *= extents[0]
    # return torch.as_tensor(labels)
...
def log_predictions2(summary_writer, metric_dict, sample_name, image, epoch,
    ...
    # bbs = torch.from_numpy(dataset._get_annotation_(batch_idx, '2d')[0])
    # bbs = normalize_labels(bbs, original_image_shape)  # sample[-2][0][0]
    # bbs = unormalize_labels(bbs, new_image_shape)
    # # self.model
    # predicted_bbs = load_predictions(predictions_dir=kitti_predictions_path,
    #                                  sample_name=sample_name, checkpoint_num=epoch,
    #                                  number_of_predictions=10)

    # predicted_bbs = normalize_labels(predicted_bbs, original_image_shape)
    # predicted_bbs = unormalize_labels(predicted_bbs, new_image_shape)
    ...
    # for bb in predicted_bbs.astype(np.uint16):
    #     x1, y1, x2, y2 = bb
    ...
    #     x1, y1, x2, y2 = bb
    #     gt_img = cv2.rectangle(gt_img, (x1, y2), (x2, y1), (255, 0, 0), 2)
    # plt.imshow(gt_img)
    # plt.show()
...
def load_predictions_and_labels(dataset, batch_idx, original_shape, new_shape, sample_name, predictions_path, epoch,
                                device='cpu'):
    bbs = load_labels(dataset, batch_idx, original_shape, new_shape, device)
    predicted_labels = load_predictions(predictions_path, sample_name, epoch, device=device)

    return bbs, predicted_labels
    # bbs = torch.from_numpy(dataset._get_annotation_(batch_idx, '2d')[0])
    # bbs = normalize_labels(bbs, original_shape)  # sample[-2][0][0]
    # bbs = unormalize_labels(bbs, [360, 1200])


def load_labels(dataset, batch_idx, original_shape, new_shape, device):
    bbs = torch.as_tensor(dataset._get_annotation(batch_idx, '2d')[0], device=device)
...
def plot_predictions_and_metrics(metric_dict, sample_name, image, epoch, bbs):
    ...
    pred_img = (image * 255).astype(np.uint8).copy()
    for bb in tp_predictions.astype(np.uint16):
        x1, y1, x2, y2 = bb
        pred_img = cv2.rectangle(pred_img, (x1, y2), (x2, y1), (0, 255, 0), 2)
    for bb in fp_predictions.astype(np.uint16):
...
def calculate_image_metrics(gt_labels, predictions, threshold=0.4):
    ...
    ious = calculate_iou(gt_labels.float(), torch.as_tensor(predictions).float())
    max_val, max_idx = ious.max(1)
    tp_mask = (max_val > threshold)
    tp = tp_mask.sum()
    tp_ids = max_idx[tp_mask]
    fp_ids = set(list(range(len(predictions)))) - set(max_idx[tp_mask].tolist())
    fp_ids = torch.tensor(list(fp_ids), dtype=torch.int64)
    tp_labels = gt_labels[tp_mask]
    fn_labels = gt_labels[~tp_mask]
    tp_predictions = predictions[tp_ids]
    fp_predictions = predictions[fp_ids]
    fp = ious.size(1) - tp
    fn = ious.size(0) - tp
...
def calculate_metrics_per_image(sample_name, gt_labels, predictions, threshold=0.4):
    ...
    # predictions[[0, 1, 2, 3, 4]] = predictions[[4, 3, 2, 1, 0]]
...
if __name__ == '__main__':
    ...
    logger = MetricLogger(path.as_posix(), ['k', 'm'])
    for i in range(10):
        logger.update('k', np.random.randn(), i)
        if np.random.randn() > 0.5:
            logger.update('m', np.random.randn(), i)
